...
public class Hashing {
	public static int search(String source, String substring) {
		...
		for (int i = substring.length() - 1; i >= 0; i--) {
			sourceHash = sourceHash + toInt(sourceArray[i]) * pow_base;
			substringHash = substringHash + toInt(substringArray[i]) * pow_base;
		...
		for (int i = 0; i < sourceArray.length - substringArray.length; i++) {
			...
			sourceHash += toInt(sourceArray[i + substringArray.length]);
		}
