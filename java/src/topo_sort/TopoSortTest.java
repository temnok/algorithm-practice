...
public class TopoSortTest {
	...
	@Test
	public void testExampleKahn() {
		Assert.assertArrayEquals(new int[] {3, 0, 2, 1}, TopoSort.topoSortKahn(new int[][]{{2, 1}, {}, {1}, {0, 2}}));

		Assert.assertArrayEquals(new int[] {}, TopoSort.topoSortKahn(new int[][]{{2, 1}, {3}, {1}, {0, 2}}));
		Assert.assertArrayEquals(new int[] {}, TopoSort.topoSortKahn(new int[][]{{0}}));
		Assert.assertArrayEquals(new int[] {}, TopoSort.topoSortKahn(new int[][]{{1}, {0}}));
	}

	@Test
	public void testRandomCasesKahn() {
		for (int test = 0; test < 10_000; test++) {
			var td = new randomTestData();

			var actual = TopoSort.topoSortKahn(td.adj);
